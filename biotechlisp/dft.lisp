...
#+ignore
(DECLARE (T (DFT-INIT          FIXNUM)
	    (DFT-FORWARD       T T T)
	    (DFT-REVERSE       T T T)
	    (DFT-REVERSE-ARRAY T T)
	    (DFT-610           T T T)
	    (DFT-618           T T T)))
...
(defun dft-reverse-bits-16 (n)
  (let ((r     n)
	(mask1 #b0101010101010101)
	(mask2 #b0011001100110011)
	(mask4 #b0000111100001111)
	(mask8 #b0000000011111111))
...
(EVAL-WHEN (EVAL)
  ...
  (DEFUN DFT-TEST-PRINT-ARRAY (ARRAY)
	...
	((>= I N))
      (DECLARE (FIXNUM I N))
      (FORMAT T "~3,1,8$~c" (aref ARRAY I) #\tab)))
...
(EVAL-WHEN (EVAL)
  ...
  #+LISPM (DEFUN RUNTIME () (TIME:MICROSECOND-TIME))

...
def _compare_by_blast_hitlist(query, xref_db, blast_out, thresh):
    cl = NcbiblastnCommandline(query=query, db=xref_db, out=blast_out,
...
def _compare_by_blast_2seq(query, subject, blast_out):
    ...
    """
    cl = NcbiblastnCommandline(query=query, subject=subject, out=blast_out,
...
def _compare_by_blast(input_ref, xref_db, blast_out, subject_blast=False,
    ...
    if blast_cmd is None:
        blast_cmd = "blastn"
    cl = NcbiblastnCommandline(cmd=blast_cmd, query=input_ref, db=xref_db,
